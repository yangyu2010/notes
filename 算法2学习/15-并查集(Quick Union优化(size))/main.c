...
The sorting used 12.567000 ms by clock()
...
void union2(struct FindUnion *fu, int v1, int v2) {
    ...
    // 就把parent2指向parent1
    // parent1的size增加
    if (fu->sizes[parent1] > fu->sizes[parent2]) {
        fu->parents[parent2] = parent1;
        fu->sizes[parent1] += fu->sizes[parent2];
    } else {
        fu->parents[parent1] = parent2;
        fu->sizes[parent2] += fu->sizes[parent1];
    }
}
...
void FindUnion(struct FindUnion *fu, int capacity) {
    ...
    fu->sizes = (int *)calloc(sizeof(int), capacity);
